...
@Component({
  selector: 'app-salesGatePass',
  templateUrl: './salesGatePassSave.component.html',
  styleUrls: ['./salesGatePassSave.component.css'],
  providers: [NotificationsComponent, dateFormatPipe, salesGatePassSaveService]
})

export class salesGatePassSaveComponent implements OnInit, AfterViewInit {
  $: any;
  deliveryForm: any;
  @Input() searchText;
  ...
  ngOnInit() {
    ...
    this.deliveryForm.get('billtype').setValue("Sales Invoice");
  ...
  getDeliverChallan(searchValue: string) {
    ...
    if (AppComponent.usertype == "\"SuperAdmin\" ") {
      ...
      this.invoiceService.getSuperStockNo(searchValue).subscribe(data => {
        this.salesinvoice = [];
        for (let i = 0; i < data.length; i++) {
          this.salesinvoice.push({ value: data[i][0], label: data[i][1] });
        }
      });
    } else {
      this.invoiceService.getDeliverChallan(searchValue, AppComponent.companyID, AppComponent.branchID, AppComponent.locRefName1, AppComponent.locrefID1, this.deliveryForm.get('billtyperefid').value).subscribe(data => {
        this.salesinvoice = [];
        for (let i = 0; i < data.length; i++) {
          this.salesinvoice.push({ value: data[i][0], label: data[i][1] });
  ...
  getDeliverChallanNo(): any {
    ...
    if (this.deliveryForm.get('salesno').value !== undefined) {
      this.invoiceService.getDeliverChallanNo(this.deliveryForm.get('salesno').value,
  ...
  showPOdata(pid: any, barandname: any, bqty: any, stripqty: any, tabqty: any, batchno: any): any {
    return this.formBuilder.group({
      ...
      stripqty: [stripqty, [Validators.pattern(textnumbers)]],
      tabqty: [tabqty, [Validators.pattern(textnumbers)]],
      totalqty: ['', []],
      dcrefid: [this.deliveryForm.get('salesno').value, []]
    });
  }
