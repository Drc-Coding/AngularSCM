...
import {genjournalEditService} from './genjournalEdit.service';
...
@Component({
  selector: 'app-genjournalEdit',
  templateUrl: './genjournalEdit.component.html',
  providers: [genjournalEditService   ,NotificationsComponent  ,  dateFormatPipe]
})
export class genjournalEditComponent implements OnInit {
  ...
  constructor(private userService: genjournalEditService, private dateformat: dateFormatPipe, private notificationsComponent: NotificationsComponent, private formBuilder: FormBuilder, config: NgbDropdownConfig, private route: ActivatedRoute) {
  ...
  ngOnInit() {
    ...
    this.userService.viewGenJournal(JSON.stringify(frmdata)).subscribe(data => { this.viewServDebitNote(data) },
      errorCode => console.log(errorCode));
    this.userService.viewGenJournalAll(JSON.stringify(frmdata)).subscribe(data => { this.editdata = data },
      errorCode => console.log(errorCode));
  ...
  onSubmit() {
    ...
    if (answer) {
      this.userService.saveGenJournal(JSON.stringify(control.value[0])).subscribe(data => { this.savevalid(data) },
        errorCode => console.log(errorCode));
    }
  ...
  deleteDebitNote() {
    ...
    if (answer) {
      this.userService.deleteGenJournal(JSON.stringify(frmdata)).subscribe(data => {this.deleteValid(data) },
        errorCode => console.log(errorCode));
