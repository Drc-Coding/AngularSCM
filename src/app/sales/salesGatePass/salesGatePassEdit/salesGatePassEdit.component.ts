...
import { salesGatePassEditService } from './salesGatePassEdit.service';
...
@Component({
  ...
  templateUrl: './salesGatePassEdit.component.html',
  providers: [salesGatePassEditService, NgbDropdownConfig, NotificationsComponent, dateFormatPipe]

})
export class salesGatePassEditComponent implements OnInit {
  ...
  constructor(private userService: salesGatePassEditService, private dateformat: dateFormatPipe, private notificationsComponent: NotificationsComponent, private formBuilder: FormBuilder, config: NgbDropdownConfig, private route: ActivatedRoute) {
  ...
  ngOnInit() {
    ...
    var frmdata = { frmint1: '', frmstr1: '', createdby: '', locrefid: this.selobj.locrefid, locname: this.selobj.locname, companyid: this.selobj.companyid };
    this.userService.getPurchaseOrderNO(JSON.stringify(frmdata)).subscribe(data => {
    this.ponos = data
    },
      errorCode => console.log(errorCode));
