...
@Injectable()
export class gatePassSaveService {
  ...
  getStockNo(searchValue: string,compid: number,brnchid: number,locname:number,locrefid: number, billtyperefid: number) {
    //Get Distributor
    return this.http.get(this.getStockNoURL+ '/'+searchValue+'/'+compid+'/'+brnchid+'/'+locname+'/'+locrefid +'/'+billtyperefid).map(response => response.json());
  }
 ...
 /* getTransfer(cid: any, bid: any, locrefid: any, locname: any) {
  ...
  getgridadta(sin:number, compid:any, branid:any, locrefid:any, locname:any){
...
return this.http.get(this.getGrid +'/'+sin+'/'+compid+'/'+branid+'/'+locrefid+'/'+locname).map(response=>response.json());
  ...
  getdeliveryproducts(dprod:number, compid:any, branid:any,locname :any,locrefid :any){
