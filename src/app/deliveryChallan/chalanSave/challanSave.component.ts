...
import { challanSaveService } from './challanSave.service';
...
@Component({
  ...
  templateUrl: './challanSave.component.html',
  styleUrls: ['./challanSave.component.css'],
  providers: [NotificationsComponent, dateFormatPipe, challanSaveService]
})

export class challanSaveComponent implements OnInit, AfterViewInit {
  ...
  getFirm() {
    ...
    if (AppComponent.usertype == "\"SuperAdmin\" ") {
      ...
      if (this.deliveryForm.get('tolocname').value == 2) {
        this.challanService.getUserWareHouse(AppComponent.companyID, AppComponent.locRefName1).subscribe(data => { this.firms = data },
          err => {
            console.log('Error Occured Get Warehouse');
          });
      ...
      if (this.deliveryForm.get('tolocname').value == 2) {
        this.challanService.getUserHospital(AppComponent.companyID, AppComponent.locRefName1).subscribe(data => { this.firms = data },
          err => {
            console.log('Error Occured Get Warehouse');
          });
      }
  ...
  getSum() {
    ...
    for (this.j = 0; this.j < setData.length; this.j++) {
      ...
      // // alert("setData getSum()" + setData.length);
      // Stripperbox = this.sbQuantity[0][0];
      // //alert("Stripperbox " + Stripperbox);
 ...
 console.log(" getData.patchValue(setData); " + this.deliveryForm.get('totaltabqty').value);
  ...
  onSubmit(): any {
    ...
    if (this.returnValid == true) {
      ...
      this.deliveryForm.get('stockno').value;
      this.deliveryForm.get('clientcdate').setValue(AppComponent.date);
      this.deliveryForm.get('billtyperefid').setValue(3);
      this.deliveryForm.get('totalqty').setValue(0);
  ...
  getStockNumber(): any {
    ...
    console.log("totaltabqty 1" + this.deliveryForm.get('totaltabqty').value);
    ...
    //alert(this.deliveryForm.get('stockno').value);
    this.challanService.getStockTransferProduct(this.deliveryForm.get('stockno').value).subscribe(data => {    
      
      this.deliveryForm.get('totaltabqty').setValue("");
    this.deliveryForm.get('totalproduct').setValue("");

 this.pushTableData(data) },
    ...
    if (data !== undefined || null || "") {
      ...
      for (this.i = 0; this.i < data.length; this.i++) {
        ...
        if (flag == 1) {
          ...
          //    alert( data[this.i][0] + "" +setData[this.x].productid  );
          /*  if (this.coltax[0] == 0) {
              this.setvatTax = true;
              $("#colvathide").show();
              $('.colvathide').show();
      ...
      console.log("totaltabqty   this.getSum();" + this.deliveryForm.get('totaltabqty').value);
    }
